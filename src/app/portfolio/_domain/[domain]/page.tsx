...
async function getPortfolioByDomain(domain: string) {
  ...
  try {
    const response = await fetch(`${baseUrl}/portfolios/domain/${domain}`, {
      method: "GET",
      headers: {
...
export default async function CustomDomainPortfolioPage({ params }: PageProps) {
  ...
  const domainToFetch = customDomain || domain;
  const portfolio = await getPortfolioByDomain(domainToFetch);
